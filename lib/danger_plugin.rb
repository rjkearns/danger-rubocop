  ...
  class DangerRubocop < Plugin
        ...
        file['offenses'].map do |offense|
          ...
          if fail_on_inline_comment
            if use_github_review
              github.review.fail(offense_message, false, file['path'], offense['location']['line'])
            else
            ...
            if use_github_review
              github.review.warn(offense_message, false, file['path'], offense['location']['line'])
            else
    ...
    def add_grouped_violation_for_each_line(offending_files, fail_on_inline_comment, report_severity, use_github_review, include_cop_names: false)
      ...
      grouped_offense_messages.each do |kargs, offense_messages|
        ...
        if fail_on_inline_comment
          if use_github_review
            github.review.fail(grouped_offense_message, false, kargs[:file], kargs[:line])
          else
          ...
          if use_github_review
            github.review.warn(grouped_offense_message, false, kargs[:file], kargs[:line])
          else
